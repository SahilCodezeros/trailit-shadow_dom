...
class Main extends React.Component {
			...
			case 'preview':
				...
				chrome.storage.local.get(["trail_web_user_tour"], async function (items) {
					...
					if (trail_web_user_tour && trail_web_user_tour.length > 0) {
						...
						if(tour.url) {
							// console.log("(trail_web_user_tour.length === tour.tourStep?1:tour.tourStep", (trail_web_user_tour.length === tour.tourStep?1:tour.tourStep));
							// console.log("trail_web_user_tour.length === tour.tourStep", trail_web_user_tour.length === tour.tourStep)
							// console.log("trail_web_user_tour.length", trail_web_user_tour.length)
							// console.log("tour.tourStep", tour.tourStep);
							if(closeContinue!==undefined) {
								let tourTData = trail_web_user_tour[(trail_web_user_tour.length === tour.tourStep?1:tour.tourStep + 1)]
								tour = await {
											tourStep: trail_web_user_tour.length === tour.tourStep?1:tour.tourStep + 1,
											currentTourType: tourTData.type,
											tourType: 'preview',
											toururl: tourTData.url
										}
							}
						}
	...
	render() {
			...
			console.log("closeContinuesssss", closeContinue);
		...
		return (
			<div className="sidepanal adadad trail_sidepanel_overlay">
				...
				<div>
					...
					{(currentTourType === 'video' && tourType === 'preview' && trailList[tourStep - 1].url === document.URL) && <VideoTour data={trailList} toggle={this.onClearToggle} tourStep={tourStep} tour={this.tourManage} tourSide={this.state.tourSide} closeButtonHandler={ this.onBackArrowClickHandler } />}
					{(currentTourType === 'audio' && tourType === 'preview' && trailList[tourStep - 1].url === document.URL) && <AudioTour data={trailList} toggle={this.onClearToggle} tourStep={tourStep} tour={this.tourManage} tourSide={this.state.tourSide} closeButtonHandler={ this.onBackArrowClickHandler } />}
				</div>
